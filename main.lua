...
local function collides(b, p)
    return not (
        b.y > p.y + PADDLE_HEIGHT or
        b.x > p.x + PADDLE_WIDTH or
        p.y > b.y + BALL_SIZE or
        p.x > b.x + BALL_SIZE
    )
end
...
function love.update(dt)
    ...
    if gameState == 'play' then
        ...
        if ball.x <= 0 then
            ...
            if player2.score > 2 then gameState = 'game over' end
        elseif ball.x >= VIRTUAL_WIDTH - BALL_SIZE then
            resetBall()
            gameState = 'serve'
            player1.score = player1.score + 1
            if player1.score > 2 then gameState = 'game over' end
        end
    end
end
...
function love.draw()
    ...
    love.graphics.setFont(LARGE_FONT)
    love.graphics.print(player1.score, VIRTUAL_WIDTH / 2 - 32, VIRTUAL_HEIGHT / 2 - 18)
    love.graphics.print(player2.score, VIRTUAL_WIDTH / 2 + 12, VIRTUAL_HEIGHT / 2 - 18)
    love.graphics.setFont(SMALL_FONT)
